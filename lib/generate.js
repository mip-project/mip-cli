...
module.exports = function generate(template, src, dest, done) {
    const metalsmith = ms(path.join(src, 'template'));
    const templatePrompts = getMeta(template, src).prompts;

    Object.assign(metalsmith.metadata(), {
        destDirName: template,
...
function ask(templatePrompts) {
    return (files, metalsmith, done) => {
        async.eachSeries(Object.keys(templatePrompts), run, done);
